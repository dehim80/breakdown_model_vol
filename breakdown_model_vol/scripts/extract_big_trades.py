...
def big_size(date):  # Записывает в файл весь месяц поминутно- трейды и кол-во сделок. если за минуту цена прошла > 150dol
    print(f'Мы в функции big_size.  date={date}') # то записывает отдельный ексель с этой минутой по еденичным сделкам
    month = '08' # сделать так чтобы в минуте в которой Ю150 дол. как будто открывалась сделка по ходу движения
    spisok = [] # и записывался в файл результат сделки, чтобы собрать статистику за пол года.Сделать с этиими же минутами ловлю ножей
